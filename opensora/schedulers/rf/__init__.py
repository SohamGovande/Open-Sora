...
from opensora.soham.config import SHOULD_PROFILE_FLOPS, SHOULD_SAVE_PICKLE
...
class RFLOW:
    ...
    def sample(
        ...
        self.prof = FlopsProfiler(model, None) if SHOULD_PROFILE_FLOPS else None

        with sg_profiler("rf") as prof:
            n = len(prompts)
            # text encoding
            ...
            for i, t in progress_wrap(enumerate(timesteps)):
                ...
                if mask is not None:
                    ...
                    t = torch.cat([t, t], 0)
                    if i == 1 and SHOULD_PROFILE_FLOPS:
                        self.prof.start_profile()
                    
                    if SHOULD_SAVE_PICKLE:
                        breakpoint()
                        torch.save(model, 'torchlog/debug_pickle/STDiT3_model.pt')
                        import pickle
                        pickle.dump((z_in, t, model_args), open('torchlog/debug_pickle/fwd_params.pkl', 'wb'))
                    ...
                    if i == 1 and SHOULD_PROFILE_FLOPS:
                        ...
                        if True:
                            self.prof.print_model_profile(profile_step=1)
